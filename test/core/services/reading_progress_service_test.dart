...
void main() {
  ...
  group('ReadingProgressService', () {
    test(
      'saveProgress persists and emits an update when page changes',
      () async {
        when(
          () => repository.getBookProgressPercent('b1'),
    ...
    test(
      'saveProgress persists but does not emit duplicate visual progress',
      () async {
        when(
          () => repository.getBookProgressPercent('b1'),
